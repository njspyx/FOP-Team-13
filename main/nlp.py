...
def getWordScores(str, repeatWeight, keyWords, keyWordWeight):
    ...
    for token in doc:
        ...
        for keyWord in keyWords:
            if keyWord.strip() != "" and keyWord in token.text.strip():
                scoreDic[token.text.strip()] = scoreDic.get(token.text.strip(), 0) + keyWordWeight
    ...
    for word in usedSentence.split(" "):
        try:
            if scoreDic[word.lower().strip()] > 0:
                scoreDic[word.lower().strip()] -= usedScore
        except KeyError:
            continue
    return scoreDic
...
if __name__ == "__main__":
    string1 = "Bataille uses theterm 'textual nihilism' to denote the bridge between sexual identity and narrativity.In a sense, Debord suggests the use of capitalist discourse to deconstructand modify class"
    ...
    with open("Sample Texts/pomo.txt", "r", encoding="utf8") as f:
        ...
        summary = summarizeText(f.read(), 10, 1, -1, keyWords, 5, False)
